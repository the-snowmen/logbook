...
3. **Write the index** at `<dir>/README.md` from
   `${CLAUDE_PLUGIN_ROOT}/skills/init/templates/index.md`, filling `{Project Name}` from `CLAUDE.md`,
...
5. **Report** the tree created and the next step: `/logbook:resume` at the start of each session;
   `/logbook:decision` / `/logbook:handoff` etc. as you work.
